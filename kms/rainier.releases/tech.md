...
- **Azure DevOps Pipelines** — primary runtime. Uses both the agentless `server` pool (for `ManualValidation@1` approval tasks) and Microsoft-hosted `ubuntu-latest` agents (for artifact publishing, policy checks, build tagging, and the notification email).
...
- **Qualys scan pipeline** (definition 800) — release policy script polls for a successful scan tied to the Developer Site resource run.
